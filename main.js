$( document ).ready(function() {
  ...
  .done(function(data) {
    $.each(data.calendar,function(){
      ...
      $('.calendar').append('<ul><li data-color="'+color+'"><a>'+date+'</a><span><p><a>'+name+'</a><a>'+place+'</a></p></span></li><ol></ol></ul>');
      $.each(this.events,function(){
        var name = this.name;
        var person = this.person;
        var time = this.time;
        var timeclass = time.replace(":", "");
        $('ul:last-child>ol','.calendar').append('<li data-color="'+color+'" class="'+timeclass+'"><span><strong>'+time+'</strong><p><a>'+name+'</a></p></span></li>');
        if(person)
